...
class TestCategory:
    ...
    def test_category_initialization(self, sample_products: list[Product]) -> None:
    ...
    def test_category_attributes_types(self, sample_products: list[Product]) -> None:
    ...
    def test_products_count_in_category(self, sample_products: list[Product]) -> None:
    ...
    def test_empty_category_products_count(self) -> None:
    ...
    def test_product_count_increment(self, sample_products: list[Product]) -> None:
        """Тест подсчета общего количества продуктов"""
        # Начинаем с 0 из-за autouse фикстуры
        assert Category.product_count == 0

        Category("Category 1", "Description 1", sample_products)
        assert Category.product_count == 3

        additional_products = [Product("New Product", "Desc", 500.0, 2)]
        Category("Category 2", "Description 2", additional_products)
    ...
    def test_combined_counts(self, sample_products: list[Product]) -> None:
