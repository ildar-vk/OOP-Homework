...
class TestMain:
    ...
    def test_main_function_execution(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        # Убрана неиспользуемая переменная lines
    ...
    def test_product_creation_like_in_main(self) -> None:
    ...
    def test_category_creation_like_in_main(self) -> None:
    ...
    def test_category_counts_like_in_main(self) -> None:
        """Тест счетчиков категорий и продуктов как в main"""
        product1 = Product("Samsung Galaxy S23 Ultra", "256GB, Серый цвет, 200MP камера", 180000.0, 5)
        product2 = Product("Iphone 15", "512GB, Gray space", 210000.0, 8)
        product3 = Product("Xiaomi Redmi Note 11", "1024GB, Синий", 31000.0, 14)

        Category("Смартфоны", "Описание", [product1, product2, product3])

        product4 = Product('55" QLED 4K', "Фоновая подсветка", 123000.0, 7)
        Category("Телевизоры", "Описание", [product4])

        assert Category.category_count == 2
        assert Category.product_count == 4

    def test_main_scenario_validation(self) -> None:
...
class TestMainScript:
    """Тесты для запуска main.py как скрипта"""
